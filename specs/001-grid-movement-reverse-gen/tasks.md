...
- [x] T007 [P] Tạo cảnh `Main.tscn` và script `Main.gd` đóng vai trò điều phối chính
...
- [x] T032 [US3] Triển khai logic sinh thực thể (`spawn_block`, `spawn_player`) trong `Main.gd`
- [x] T033 [US3] Thực hiện quy trình ánh xạ `LevelState` sang các Node trực quan tại `_ready()`
- [x] T034 [US3] Truyền tham chiếu `grid_logic` cho mọi thực thể được sinh ra
- [x] T035 [US3] Thiết lập nhãn HUD ban đầu (ẩn WinLabel, reset Score)
- [x] T036 Kết nối tín hiệu `win_condition_met` từ `Block.gd` tới `Main.gd` để hiển thị WinLabel
