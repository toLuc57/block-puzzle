...
func test_player_blocked_by_water_obstacle_and_outer_wall():
	var water_pos = Vector2i(5, 5)
	...
	assert_false(grid_logic.can_player_enter(water_pos), "Player should be blocked by water cells")
